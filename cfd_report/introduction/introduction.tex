...
where $f^*$ is the current best observed value of the aerodynamic efficiency objective. The EI quantifies the expected improvement over the current optimum, balancing exploration (high surrogate uncertainty) against exploitation (low surrogate mean). As the surrogate is refined with each DES evaluation, EI should decay monotonically; convergence is declared when EI falls below a threshold.
...
where $\lambda = 1/3$ penalises drag three times more than it rewards downforce, reflecting a typical high-speed cornering compromise between drag on the straights and downforce through fast corners. Minimising $f_1$ favours configurations that generate meaningful downforce at a manageable drag cost.
