...
Five L2 solves at fixed $\alpha_\mathrm{diff}=0^\circ$ across the slant-angle range are in progress; results will populate Table~\ref{tab:slant_sweep} and Figure~\ref{fig:slant_trend}.

\begin{table}[h]
  \centering
  \caption{Computed $C_d$ vs slant angle at $\alpha_\mathrm{diff}=0^\circ$ (L2 mesh, kOmegaSST). Ahmed (1984) experimental values included for trend comparison.}
  \label{tab:slant_sweep}
  \begin{tabular}{lccc}
    \toprule
    $\alpha_\mathrm{slant}$ (°) & $C_d$ (computed, L2) & $C_d$ (Ahmed 1984\textsuperscript{a}) & Flow regime \\
    \midrule
    12.5 & \textit{pend.} & ${\approx}$0.230 & Sub-critical, attached \\
    20.0 & \textit{pend.} & ${\approx}$0.255 & Sub-critical, growing vortex \\
    25.0 & \textit{pend.} & 0.299  & Lienhart reference \cite{lienhart2002} \\
    30.0 & \textit{pend.} & ${\approx}$0.380 & Near-critical, maximum $C_d$ \\
    35.0 & \textit{pend.} & ${\approx}$0.300 & Super-critical, fully separated \\
    \bottomrule
    \multicolumn{4}{l}{\textsuperscript{a}Approximate values from Ahmed et al.~\cite{ahmed1984} Fig.~11; geometry: $\alpha_\mathrm{diff}=0^\circ$, no raised floor.}\\
  \end{tabular}
\end{table}
